...
function cms_tpv_admin_head() {
	...
	<script type="text/javascript">
		...
		var CMS_TPV_CAN_DND = "<?php echo ( current_user_can( CMS_TPV_MOVE_PERMISSION )  || current_user_can( 'manage_options' ) ) ? "dnd" : "" ?>";
...
function cms_tpv_is_one_of_our_pages() {
	...
	return ( $current_screen->id == 'pages_page_cms-tpv-page-page' || $current_screen->id == 'page' );
...
function cms_tpv_replace_pageparent_metabox( $post_type, $post ) {
	...
	// Add a new meta box for the Page Template
	$templates = get_page_templates();
	$post_type_object = get_post_type_object( $post_type );

	if ( 'page' === $post_type && is_array( $templates ) && ( count( $templates ) > 0 ) ) {
		add_meta_box(
			'cmstpvpagetemplatediv',
			sprintf( "%s Attributes", $post_type_object->labels->singular_name ),
			'cms_tpv_pageattributes_html',
			$post_type,
			'side',
			'core',
			$post_type
...
function cms_tpv_pageattributes_html( $post_type ) { 
	$current_page_template = get_page_template_slug($GLOBALS['post']->ID); 
	?>
	<p>
		<label for="page_template">Template</label>
		<select name="page_template" id="page_template">
			<option value='default'>Default Template</option>
			<?php page_template_dropdown( $current_page_template ); ?>
		</select>
	</p>

	<p>
		<label for="parent_id">Parent ID</label>
		<input name="parent_id" type="text" size="4" id="parent_id" value="<?php echo $GLOBALS['post']->post_parent ?>">
	</p>

	<p>
		<label for="menu_order">Page Order</label>
		<input name="menu_order" type="text" size="4" id="menu_order" value="<?php echo $GLOBALS['post']->menu_order ?>">
	</p>
<?php
}
...
function cms_tpv_admin_menu() {
	...
	add_submenu_page('edit.php?post_type=page', 'Tree View', 'Tree View', 'edit_pages', 'cms-tpv-page-page', 'cms_tpv_pages_page');
	add_submenu_page('edit.php?post_type=page', 'New Page', 'New Page', 'edit_pages', 'post-new.php?post_type=page');
		//}
	// }
...
function cms_tpv_move_page() {
	...
	*/
	...
	if ( !current_user_can( CMS_TPV_MOVE_PERMISSION ) && !current_user_can( 'manage_options') ) 
		die("Error: you don't have permission");
