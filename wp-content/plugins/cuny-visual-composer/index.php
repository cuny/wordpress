...
class cuny_visual_composer{
  public static function init(){
    ...
    add_action( 'init', array( __CLASS__, 'shortcode_map' ) );
  ...
  public static function admin_enqueue_scripts_and_styles( $hook ) {
  	wp_register_style( 'cuny_vc_style', plugins_url( 'css/style.css', __FILE__ ), array( 'js_composer' ), null );
  	wp_enqueue_style( 'cuny_vc_style' );

  	if ( $hook == 'post.php' ) {
	  	wp_register_script( 'cuny_vc_script', plugins_url( '/js/functions.js', __FILE__ ), array( 'wpb_js_composer_js_view' ), null, true );
	...
	public static function section_header_shortcode( $atts, $content = null ) {
		...
		$el_class = !empty($el_class)?" $el_class":'';
		if (!empty($title)){
			$output = "<h$level class='section-header$el_class'>$title <a class='follow' href='$follow_url'>Follow <i class='$icon'></i></a></h$level>";
		}
		return $output;
	}
...
// Init the plugin
add_action( 'plugins_loaded', array('cuny_visual_composer', 'init'), 50 );
